...
#MODEL4
train_copy<-train
train_copy$churn_risk_score <- ifelse(train_copy$churn_risk_score ==4, 1, 0)
...
fit.lasso.cv <- cv.glmnet(x, y, nfold=10, alpha=1, lambda=model4$lambda) 
fit.lasso.param <- fit.lasso.cv$lambda.1se
model4 <- glmnet(x,y,alpha = 1,lambda =fit.lasso.param )

#MODEL5
train_copy<-train
train_copy$churn_risk_score <- ifelse(train_copy$churn_risk_score ==5, 1, 0)
...
fit.lasso.cv <- cv.glmnet(x, y, nfold=10, alpha=1, lambda=model5$lambda) 
fit.lasso.param <- fit.lasso.cv$lambda.1se
model5 <- glmnet(x,y,alpha = 1,lambda =fit.lasso.param )



#Predict
x.test <- model.matrix(churn_risk_score ~., test)[,-1]
prob1 <- predict(model1, x.test, type="response")

x.test <- model.matrix(churn_risk_score ~., test)[,-1]
prob2 <- predict(model2, x.test, type="response")

x.test <- model.matrix(churn_risk_score ~., test)[,-1]
prob3 <- predict(model3, x.test, type="response")

x.test <- model.matrix(churn_risk_score ~., test)[,-1]
prob4 <- predict(model4, x.test, type="response")

x.test <- model.matrix(churn_risk_score ~., test)[,-1]
prob5 <- predict(model5, x.test, type="response")





prob_result<-data.frame(prob1,prob2,prob3,prob4,prob5)
prob_result['result']<- names(prob_result)[1:5][max.col(prob_result[1:5], "first")]
prob_result$result<-ifelse(prob_result$result=="s0",1,ifelse(prob_result$result=="s0.1",2
                                                             ,ifelse(prob_result$result=="s0.2",3
                                                                     ,ifelse(prob_result$result=="s0.3",4
                                                                             ,5))))
